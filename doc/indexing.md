...
如果你愿意，我可以：
- 1) 在 `be/model/search.py` 中添加/展示如何构造 `text`/FTS 查询以及如何优先使用索引（小改动）。
- 2) 把初始化索引的命令以脚本形式加入 `script/`（如 `script/init_indexes.sh` 或 PowerShell 版本），便于部署时执行。

请选择下一步（我可以立即实现其中一项并运行快速验证）。
